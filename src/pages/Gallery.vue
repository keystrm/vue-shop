...
watch(loaderRef, (newValue, oldValue) => {
  ...
  .gallery {
    display: grid;
    grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); /* Adjust minmax() for tile size */
    gap: 16px;
  ...
  .tile {
    ...
    align-items: center;
    height: 300px; /* Fixed height, or min-height based on your design */
  ...
  .loading-placeholder {
    width: 100%;
    height: 100%;
    background: linear-gradient(130deg, #e4e4e4 40%, #f0f0f0 50%, #e4e4e4 60%); /* Animated loading effect */
    background-size: 200% 100%;
    animation: loading 1.5s infinite linear;
  }
  
  .media-img {
    width: 100%; /* Ensure images fit within the tile */
